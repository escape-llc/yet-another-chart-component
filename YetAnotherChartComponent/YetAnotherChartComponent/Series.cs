...
namespace eScapeLLC.UWP.Charts {
	...
	public abstract class DataSeries : ChartComponent, IProvideValueExtents, IProvideCategoryExtents {
		...
		/// <summary>
		/// Calculate a transform matrix for given area, based on the axes.
		/// Y-axis goes "up" as values increase, the opposite of device coordinates.
		/// Matrix.M11 = scalex
		/// Matrix.M22 = -scaley
		/// Matrix.M12 = Matrix.M21 = 0
		/// The matrix created is equivalent to the final matrix generated by the following TransformGroup:
		///		var tg = new TransformGroup();
		///		tg.Children.Add(new ScaleTransform() { ScaleX = scalex, ScaleY = -scaley });
		///		tg.Children.Add(new TranslateTransform() { X = area.Left + xoffset * scalex, Y = area.Top + yoffset * scaley });
		///		Geometry.Transform = tg;
		/// </summary>
		/// <param name="area">The rectangle to make transform against. The Top and Left are included in Translate.  The Width and Height are included in the Scale.</param>
		/// <param name="offsetx">Offset X in "outer" units; internally scaled by X-scale.</param>
		/// <param name="offsety">Offset Y in "outer" units; internally scaled by Y-scale.</param>
		/// <returns></returns>
		protected virtual Matrix MatrixFor(Rect area, double offsetx, double offsety) {
			var scalex = CategoryAxis.ScaleFor(area.Width);
			var scaley = ValueAxis.ScaleFor(area.Height);
			var matx = new Matrix(scalex, 0, 0, -scaley, area.Left + offsetx*scalex, area.Top + offsety*scaley);
			return matx;
		}
		#endregion
	}
	...
	public class LineSeries : DataSeries, IDataSourceRenderer, IProvideLegend, IRequireEnterLeave, IRequireTransforms {
		...
		public void Transforms(IChartRenderContext icrc) {
			...
			var matx = MatrixFor(icrc.Area, 0, ValueAxis.Maximum);
	...
	public class MarkerSeries : DataSeries, IDataSourceRenderer, IProvideLegend, IRequireEnterLeave, IRequireTransforms {
		...
		protected void TransformMarker(Geometry mk, Matrix gmatx) {
			if(mk is EllipseGeometry) {
				...
				var factorx = MarkerWidth*gmatx.M11;
				var factory = factorx/gmatx.M22;
				var matx = new Matrix(gmatx.M11 * MarkerWidth, 0, 0, gmatx.M11 * MarkerWidth, gcx.X, gcx.Y);
